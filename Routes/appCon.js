...
//                                 final100.forEach((element) => {
//                                         connection.query(
//                                             "select name from products where id=$1",
//                                             [final100[i].id],
//                                             (err, result3) => {
//                                                 if (!err) {
//                                                     final100[i].productname = result3.rows[0].name;
//                                                     i++;
//                                                 }
//                                             });
//                                         // array.push(final);
    ...
    connection.query(
        ...
        (err, result) => {
            if (!err) {
                ...
                var i=0;
                console.log("mai foreach chayaya issk bad");
                finalResult.forEach(
                    (element) => {
                        connection1.query("select * from places where id = $1",
                            [element.source_id],
                            (err, result100) => {
                                if(!err){
                                    console.log("mai loop me hu");
                                    element.source_id = result100.rows;
                                    finalResult[i]["source_id"] = result100.rows;
                                    // console.log(finalResult[i].source_id);
                                    flag = 1;
                                    console.log(flag);//1
                                }
                            });
                ...
                connection.query(
                  ...
                  (err, result3) => {
                    if (!err) {
                      ...
                      console.log(err);
                      res.send("-1");
>>>>>>> aab084416117b894e2ff1cd2164f489cb9e85add
                    }
                );
                console.log(flag);//0
            }
    ...
    if (flag==1){
        console.log("mai 1 hu");
        console.log(finalResult);
    }
    else
    {
        console.log("me 0 hun");
    }
});
